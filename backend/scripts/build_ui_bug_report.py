...
COVERAGE = [
    ...
    ('T-726', 'My Work', 'Approval moment - server rule', 'Desktop 1440x900', 'Owner', 'The approvals feed holds a before-done task only while it waits, a before-start task until approved', 'Data', 'PASS', 'GET /tasks?view=approvals: 7 rows, 0 break the rule; 17 unit tests on the stage helpers and the query', 'ASK-28'),
    ('T-727', 'My Work', 'New Task - Needs approval choice', 'Desktop 1440x900', 'Owner', 'Three choices, No by default; approver picker and hint follow the choice', 'Form', 'PASS', 'No / Before work starts / Before it is marked done; approver hidden on No; labels fit', 'ASK-28'),
    ('T-728', 'My Work', 'New Task - payload', 'Desktop 1440x900', 'Owner', 'Choosing before it is marked done sends approval_stage close', 'Form', 'PASS', 'approval_required true, approval_stage close, approver_id null (POST answered by the script)', 'ASK-28'),
    ('T-729', 'My Work', 'Approval pills on cards', 'Desktop 1440x900', 'Sales (made-up tasks)', 'Cards say which approval a task carries', 'Views', 'PASS', 'Approval to start / Approval to close (amber, waiting) / Needs approval to close (quiet, being worked)', 'ASK-28'),
    ('T-730', 'My Work', 'Before done - Complete asks', 'Desktop 1440x900', 'Sales (made-up task)', 'The doer works freely; Complete sends the task for approval', 'Flow', 'PASS', 'No lock banner; PATCH status done; toast Sent for approval (answer played by the script)', 'ASK-28'),
    ('T-731', 'My Work', 'Before done - waiting', 'Desktop 1440x900', 'Sales (made-up task)', 'While it waits the doer sees who will close it and no Complete button', 'Flow', 'PASS', 'Waiting for approval banner; Complete and approval box hidden', 'ASK-28'),
    ('T-732', 'My Work', 'Before done - sent back', 'Desktop 1440x900', 'Sales (made-up task)', 'Changes requested shows the reason and Complete is offered again', 'Flow', 'PASS', 'Banner with the approver reason; Complete shown', 'ASK-28'),
    ('T-733', 'My Work', 'Before start - still locks', 'Desktop 1440x900', 'Sales (made-up task)', 'Approval before work starts keeps its lock', 'Flow', 'PASS', 'Lock banner, no Complete', 'ASK-28'),
    ('T-734', 'My Work', 'Approvals - both moments', 'Desktop 1440x900', 'Owner (made-up list)', 'The Approvals view lists both kinds with labels and one count', 'Views', 'PASS', '2 cards oldest first, pills Approval to start / Approval to close, badge 2', 'ASK-28'),
    ('T-735', 'My Work', 'Approve closes a before-done task', 'Desktop 1440x900', 'Owner (made-up task)', 'The approver box says the work is finished and Approve closes it', 'Flow', 'PASS', 'Box: marked this task complete; POST approve; toast Approved - task closed; before-start box unchanged', 'ASK-28'),
    ('T-736', 'My Work', 'My team - the rule', 'Desktop 1440x900', 'Finance (manager of sai)', 'The team feed holds only tasks a direct report is doing or helping on', 'Data', 'PASS', 'GET /tasks?view=team: 16 tasks (15 open), 0 without a report on them; 9 unit tests', 'ASK-28'),
    ('T-737', 'My Work', 'My team - switcher', 'Desktop 1440x900', 'Finance (manager)', 'A manager gets My team in the view switcher', 'Views', 'PASS', 'Tasks | Asked by me | My team | Workflows', 'ASK-28'),
    ('T-738', 'My Work', 'My team - list', 'Desktop 1440x900', 'Finance (manager)', 'The list is exactly the team open work, in the URL, kept on refresh', 'Views', 'PASS', '15 cards = 15 open from the API; ?view=team survives reload; not saved as default', 'ASK-28'),
    ('T-739', 'My Work', 'My team - filters', 'Desktop 1440x900', 'Finance (manager)', 'Person lists the team (no Me); AI priority off', 'Views', 'PASS', 'Person: All people 15, sai 15, two helpers-on tasks; AI toggle hidden', 'ASK-28'),
    ('T-740', 'My Work', 'My team - open a report task', 'Desktop 1440x900', 'Finance (manager)', 'The manager can open a report task and its activity', 'Permissions', 'PASS', 'GET /tasks/{id} 200 and /activity 200; a production user not on it still gets 403', 'ASK-28'),
    ('T-741', 'My Work', 'My team - drawer in own department', 'Desktop 1440x900', 'Finance (manager)', 'On a report task in the manager department the existing rule still gives full controls', 'Permissions', 'PASS', 'Complete shown; Log note, Hand off, Escalate offered', 'ASK-28'),
    ('T-742', 'My Work', 'My team - drawer outside own department', 'Desktop 1440x900', 'Finance (made-up task)', 'On a report task outside the manager department the drawer is note-only', 'Permissions', 'PASS', 'Hint: sai is doing this task; no Complete; only Log note', 'ASK-28'),
    ('T-743', 'My Work', 'My team - no reports', 'Desktop 1440x900', 'Sales', 'Someone with no reports gets no My team, and a My team link lands on My Tasks', 'Views', 'PASS', 'No segment; API returns 0; ?view=team dropped, My Work pressed', 'ASK-28'),
    ('T-744', 'My Work', 'My team - owner', 'Desktop 1440x900', 'Owner', 'The owner keeps All Tasks and a My team link lands on My Tasks', 'Views', 'PASS', 'My Tasks | Asked by me | All Tasks | Approvals | Workflows; ?view=team dropped in about 1.6 s', 'ASK-28'),
    ('T-745', 'My Work', 'Task link - not in the list', 'Desktop 1440x900', 'Finance (manager of sai)', 'A link to a task the person may open but that is not in their list opens its drawer', 'Flow', 'PASS', 'sai task from Sunita My Tasks: drawer opens, no card in the list (was: nothing)', 'ASK-28'),
    ('T-746', 'My Work', 'Task link - finished task', 'Desktop 1440x900', 'Finance (manager)', 'A link to a finished task opens it', 'Flow', 'PASS', 'Completed task of sai opens (was: nothing)', 'ASK-28'),
    ('T-747', 'My Work', 'Task link - while on My Work', 'Desktop 1440x900', 'Finance (manager)', 'A task link followed while My Work is already open opens the task', 'Flow', 'PASS', 'In-app navigation to ?task= opens the drawer (was: nothing)', 'ASK-28'),
    ('T-748', 'My Work', 'Task link - older focus form', 'Desktop 1440x900', 'Finance (manager)', '?focus=task:<id> links from the Brief and phone screens open the task', 'Flow', 'PASS', 'Drawer opens; link cleared on close (was: ignored)', 'ASK-28'),
    ('T-749', 'My Work', 'Task link - closing', 'Desktop 1440x900', 'Finance (manager)', 'Closing the drawer takes the link out of the address; refresh keeps it closed', 'Flow', 'PASS', 'url {} after Escape and after reload', 'ASK-28'),
    ('T-750', 'My Work', 'Task link - inside My team', 'Desktop 1440x900', 'Finance (manager)', 'When the task is in the list, the list card opens (no stand-in)', 'Flow', 'PASS', '?view=team&task=<id>: My team pressed, card drawer', 'ASK-28'),
    ('T-751', 'My Work', 'Task link - missing task', 'Desktop 1440x900', 'Owner', 'A link to a task that no longer exists says so and can be dismissed', 'Errors', 'PASS', 'This task no longer exists + Dismiss clears the link (was: Access restricted)', 'ASK-28'),
    ('T-752', 'My Work', 'Task link - no access', 'Desktop 1440x900', 'Production', 'A link to a task the person may not open says Access restricted and opens nothing', 'Permissions', 'PASS', 'API 403; banner reason denied; no drawer', 'ASK-28'),
    ('T-753', 'My Work', 'View survives refresh', 'Desktop 1440x900', 'Owner', 'The chosen view is in the address and survives refresh', 'Views', 'PASS', 'Workflows: ?view=workflows, still Workflows after reload (was: My Tasks)', 'ASK-28'),
    ('T-754', 'My Work', 'Back and Forward', 'Desktop 1440x900', 'Owner', 'Back and Forward move between the views visited', 'Views', 'PASS', 'My Tasks > Asked > All; Back Asked, Back My Tasks, Forward Asked (was: Back left the page)', 'ASK-28'),
    ('T-755', 'My Work', 'View links', 'Desktop 1440x900', 'Owner', '?view=all and ?view=approvals land on those views', 'Views', 'PASS', 'All Tasks pressed; Approvals hub shown', 'ASK-28'),
    ('T-756', 'My Work', 'Task link - phone', 'Mobile 390x844', 'Finance (manager)', 'A task link not in the list opens on a phone', 'Flow', 'PASS', 'Drawer opens; no overflow', 'ASK-28'),
    ('T-757', 'My Work', 'Phone - view pill', 'Mobile 390x844', 'Owner', 'One pill names the view on screen and shows how many approvals wait', 'Views', 'PASS', 'Pill: My Tasks with badge 6 = approvals waiting; header fits, no overflow', 'ASK-28'),
    ('T-758', 'My Work', 'Phone - view sheet by role', 'Mobile 390x844', 'Owner / Sales / Finance manager', 'The sheet lists exactly the views each person has', 'Views', 'PASS', 'Owner: My Tasks, Asked by me, All Tasks, Approvals 6; Sales: My Tasks, Asked by me; Sunita: + My team', 'ASK-28'),
    ('T-759', 'My Work', 'Phone - picking a view', 'Mobile 390x844', 'Owner', 'Picking a view changes the list and the address, closes the sheet, Back returns', 'Views', 'PASS', 'Asked 53 = API 53; All 129 = API 129; Approvals hub; Back to All Tasks', 'ASK-28'),
    ('T-760', 'My Work', 'Phone - My team', 'Mobile 390x844', 'Finance (manager)', 'A manager reaches My team on a phone', 'Views', 'PASS', '15 cards = 15 open from the API; no overflow', 'ASK-28'),
    ('T-761', 'My Work', 'Phone - New Task form', 'Mobile 390x844', 'Owner', 'New Task is full-screen, nothing past the edge, Create in reach, right payload', 'Form', 'PASS', '390x844; approval choice wraps (was cut off); POST approval_stage close (answered by the script)', 'ASK-28'),
    ('T-762', 'My Work', 'Phone - task drawer', 'Mobile 390x844', 'Owner', 'A task drawer fits the phone without sideways scrolling', 'Flow', 'PASS', 'Drawer inside the 390px screen; inner overflow 0', 'ASK-28'),
    ('T-763', 'Tasks API', 'Real saves - Asked by me', 'Backend (isolated DB)', 'Sales', 'A non-owner creates a task for someone else and still sees it; may note, not hand off', 'Data', 'PASS', 'test_task_management_e2e: asked list, get, note saved, hand-off 403', 'ASK-28'),
    ('T-764', 'Tasks API', 'Real saves - approve before start', 'Backend (isolated DB)', 'Finance (named approver)', 'A non-owner named approver is told, sees it, approves; the doer is locked until then', 'Data', 'PASS', 'blocked/pending -> 403 for doer -> approve -> todo/approved -> doer starts', 'ASK-28'),
    ('T-765', 'Tasks API', 'Real saves - approve before done', 'Backend (isolated DB)', 'Operations doer / Finance approver', 'Complete asks, changes go back with the reason, approve closes, reopen needs sign-off again', 'Data', 'PASS', 'review/pending + 1 notification; 409 on repeat; rejected in_progress; approve done/approved 100; reopen clears', 'ASK-28'),
    ('T-766', 'Tasks API', 'Approver must be able to approve', 'Backend (isolated DB)', 'Sales creating', 'A named approver without approval access is refused; role settings can grant it', 'Permissions', 'PASS', '400 and nothing saved; production role given approvals in role settings -> accepted; owner always; outsider dropped', 'ASK-28'),
    ('T-767', 'Tasks API', 'Real saves - My team', 'Backend (isolated DB)', 'Sales (manager of operations)', 'A manager sees, opens and notes on a report task, and nothing else', 'Permissions', 'PASS', 'team = doing + helping; other task 403 for get and activity; hand-off 403', 'ASK-28'),
    ('T-768', 'My Work', 'Drawer - Asked by', 'Desktop 1440x900', 'Owner', 'The drawer names who asked for the task', 'Views', 'PASS', 'Asked by <name>; Asked by You on a task the owner created', 'ASK-28'),
    ('T-769', 'My Work', 'Drawer - approver and moment', 'Desktop 1440x900', 'Owner', 'The drawer names the approver and when approval happens', 'Views', 'PASS', 'Approval before work starts / before it is marked done: <approver or anyone with approval access>', 'ASK-28'),
    ('T-770', 'My Work', 'Drawer - picked automatically', 'Desktop 1440x900', 'Owner (made-up task)', 'A team task says it went to the member with the fewest open tasks', 'Views', 'PASS', 'Picked automatically: fewest open tasks in Sales; no supporting-employee line', 'ASK-28'),
    ('T-771', 'My Work', 'New Task - team toast', 'Desktop 1440x900', 'Owner', 'Giving a task to a team says who it went to and why', 'Form', 'PASS', 'Assigned to Priya Nair: fewest open tasks in Sales (POST answered by the script); no Supporting employee field', 'ASK-28'),
    ('T-772', 'My Work', 'Drawer - people lines on a phone', 'Mobile 390x844', 'Owner', 'The people lines fit a phone', 'Views', 'PASS', 'Lines shown; no sideways overflow', 'ASK-28'),
    ('T-773', 'Tasks API', 'Supporting employee removed', 'Backend (isolated DB)', 'Sales', 'An old app sending support_id still creates the task; the field is not kept and grants no access', 'Data', 'PASS', 'created; no support_id stored; would-be supporter gets 403', 'ASK-28'),
    ('T-774', 'Tasks API', 'Auto-assigned note', 'Backend (isolated DB)', 'Sales / Owner', 'A team task records how it was routed, and a reassign clears it', 'Data', 'PASS', 'auto_assigned {role operations, fewest_open_tasks}; cleared after PATCH assignee; named doer never auto', 'ASK-28'),
    ('T-775', 'Tasks API', 'Helpers get notified', 'Backend (isolated DB)', 'Operations lead / Finance helper', 'A helper hears status changes and overdue reminders like the lead', 'Data', 'PASS', 'status change -> helper + asker notified; overdue reminder -> lead + helper', 'ASK-28'),
    ('T-776', 'My Work', 'Stages on cards', 'Desktop 1440x900', 'Owner', 'Cards show To do / Doing / Done, never the old status words', 'Views', 'PASS', '129 chips: To do, Doing; none of Not Started, In Progress, Under Review, Pending Approval', 'ASK-28'),
    ('T-777', 'My Work', 'Status filter - stages and flags', 'Desktop 1440x900', 'Owner', 'The filter lists stages, flags and lenses, and counts match the server', 'Views', 'PASS', 'To do 116 = 116, Doing 13 = 13, Needs approval 39 = 39', 'ASK-28'),
    ('T-778', 'My Work', 'Status filter - old links', 'Desktop 1440x900', 'Owner', 'An old ?status=blocked link lands on Needs approval', 'Views', 'PASS', 'Status: Needs approval 39', 'ASK-28'),
    ('T-779', 'My Work', 'Status picker', 'Desktop 1440x900', 'Owner', 'The drawer status picker offers the two stages', 'Form', 'PASS', 'To do, Doing', 'ASK-28'),
    ('T-780', 'My Work', 'Waiting on a supplier', 'Desktop 1440x900', 'Owner (made-up task)', 'Waiting on someone takes a typed name and shows who and since when', 'Flow', 'PASS', 'PATCH waiting_on {name}; box Waiting on Kumar Fabrics (supplier) since today; card flag + Doing chip', 'ASK-28'),
    ('T-781', 'My Work', 'Waiting on a colleague / stop', 'Desktop 1440x900', 'Owner (made-up task)', 'A colleague can be picked, and Stop waiting ends it', 'Flow', 'PASS', 'PATCH waiting_on {user_id}; Stop waiting PATCH {}; back to Doing', 'ASK-28'),
    ('T-782', 'My Work', 'Waiting on - filter', 'Desktop 1440x900', 'Owner', 'Waiting on someone lists the waiting tasks', 'Views', 'PASS', 'stored waiting 1 + the made-up one = 2 cards', 'ASK-28'),
    ('T-783', 'My Work', 'Stages and waiting on a phone', 'Mobile 390x844', 'Owner (made-up task)', 'The phone bar shows the two stages and the waiting box fits', 'Views', 'PASS', 'pills To do, Doing (pressed); Waiting on ... (3 days); no overflow', 'ASK-28'),
    ('T-784', 'Tasks API', 'Real saves - waiting on', 'Backend (isolated DB)', 'Operations doer / Finance colleague', 'Waiting on stores who and since, tells a colleague, gives note-only access, ends on a status change', 'Data', 'PASS', 'status waiting + waiting_on; 1 notification; note ok, hand-off 403; name switch drops access; in_progress clears; {} stops; blank 400; done 400', 'ASK-28'),
    ('T-785', 'Tasks API', 'Waiting on - start lock', 'Backend (isolated DB)', 'Operations doer', 'A task waiting for approval to start cannot be set waiting', 'Permissions', 'PASS', '403; stays blocked', 'ASK-28'),
    ('T-786', 'Tasks API', 'Open work counts', 'Backend (unit)', '-', 'Operating Score counts waiting and under review as open', 'Data', 'PASS', '_is_open_task true for todo, blocked, in_progress, waiting, review', 'ASK-28'),
    ('T-787', 'Tasks API', 'New task permissions are opt-in', 'Backend (unit)', '-', 'Assign tasks to anyone and See all tasks exist and no role gets them by default', 'Permissions', 'PASS', 'not in base or role defaults; owner holds both', 'ASK-28'),
    ('T-788', 'My Work', 'Permissions the server applies', 'Desktop 1440x900', 'Owner / Sales / Production / Finance', '/auth/me carries effective_permissions; only the owner holds the new ones', 'Permissions', 'PASS', 'sales 6, production 6, finance 8 permissions; new ones owner-only', 'ASK-28'),
    ('T-789', 'My Work', 'All Tasks - who gets it', 'Desktop + Mobile', 'Owner / Sales / Production / Finance', 'All Tasks only for the owner (finance does not get it); a manager keeps My team', 'Views', 'PASS', 'desktop switcher and phone view sheet per role; Sunita: Tasks, Asked by me, My team', 'ASK-28'),
    ('T-790', 'My Work', 'All Tasks - link', 'Desktop 1440x900', 'Sales / Production / Finance', 'A ?view=all link opened by someone without See all tasks lands on their own tasks', 'Views', 'PASS', 'no All Tasks segment after the link', 'ASK-28'),
    ('T-791', 'My Work', 'New Task - Assign to by role', 'Desktop 1440x900', 'Owner / Sales / Production / Finance', 'Assign to lists exactly the people and teams the person may give work to', 'Form', 'PASS', 'owner 12 + 3 teams; sales 7 + Sales; production self only; finance self + sai + Finance', 'ASK-28'),
    ('T-792', 'My Work', 'New Task - helpers by role', 'Desktop 1440x900', 'Owner / Sales / Production / Finance', 'Helpers list follows the same rule; an empty list says so', 'Form', 'PASS', 'helpers = allowed people minus the doer; production sees a note instead of an empty dropdown', 'ASK-28'),
    ('T-793', 'My Work', 'Grant both permissions (mocked)', 'Desktop 1440x900', 'Sales (mocked /auth/me)', 'Given the permissions, All Tasks appears and Assign to lists everyone', 'Permissions', 'PASS', 'All Tasks requests /tasks?mine=false; 12 people + 3 teams', 'ASK-28'),
    ('T-794', 'Team', 'Member dialog - new toggles', 'Desktop 1440x900', 'Owner', 'The member dialog lists Assign tasks to anyone and See all tasks, off by default', 'Permissions', 'PASS', 'both toggles present, aria-pressed false', 'ASK-28'),
    ('T-795', 'My Work', 'No refused requests', 'Desktop + Mobile', 'Owner / Sales / Production / Finance', 'Loading My Work and New Task makes no request the server refuses', 'Permissions', 'PASS', '0 x 403 per role on desktop and phone', 'ASK-28'),
    ('T-796', 'Tasks API', 'Real saves - who may give work to whom', 'Backend (isolated DB)', 'Sales / Sales + grant / Owner', 'Self, own team and reports allowed; anyone else refused as doer, team or helper, on create, change and reassign; the grant lifts it', 'Permissions', 'PASS', '403 with the name and nothing saved; removing a helper free; Manage Team alone refused; tasks_assign_any allowed', 'ASK-28'),
    ('T-797', 'Tasks API', 'Real saves - create needs Tasks', 'Backend (isolated DB)', 'Sales without tasks', 'Creating a task without the Tasks permission is refused', 'Permissions', 'PASS', '403 You do not have access to create tasks; nothing saved', 'ASK-28'),
    ('T-798', 'Tasks API', 'Real saves - See all tasks', 'Backend (isolated DB)', 'Finance / Sales / Sales + grant', 'See all tasks is opt-in; finance keeps its lane', 'Permissions', 'PASS', 'finance lane only and 403 on another task; grant lists and opens everything; My Tasks unchanged', 'ASK-28'),
    ('T-799', 'Tasks API', 'Who may change a task - rule', 'Backend (unit)', '-', 'Rights by relationship: doer, helper, asker, manager, approver, See all tasks, Manage Team, stranger', 'Permissions', 'PASS', 'helper moves but does not finish; proof only asker or owner; approver and See all tasks change nothing', 'ASK-28'),
    ('T-800', 'Tasks API', 'Real saves - who may change a task', 'Backend (isolated DB)', 'Sales / Sales2 / Ops / Production / Finance', 'Refusals and allowed changes are stored exactly as the rule says', 'Permissions', 'PASS', 'stranger refused even for an unchanged value; helper refused done, cancel, reopen, people, priority; doer refused proof; asker switches proof off; manager finishes', 'ASK-28'),
    ('T-801', 'My Work', 'Drawer - doer and helper', 'Desktop + Mobile', 'Owner / Sales / Production / Finance', 'Doer gets stage and Complete; a helper gets stage and a line that the doer marks it done', 'Permissions', 'PASS', 'Complete hidden for helpers on desktop and phone (the phone Cancel link was removed in 8fbe532)', 'ASK-28'),
    ('T-802', 'My Work', 'Drawer - person who asked and manager', 'Desktop + Mobile', 'Sales / Finance manager', 'The person who asked and the manager run the task: stage, Complete, people', 'Permissions', 'PASS', 'no follow-only hint; hand-off stays note-only when not on the task', 'ASK-28'),
    ('T-803', 'My Work', 'Drawer - approver, waited on, unrelated', 'Desktop + Mobile', 'Sales / Production / Finance', 'Followers see where the work is and leave notes; no stage, Complete or people controls', 'Permissions', 'PASS', 'follow-only hint shown; update form offers Log note only', 'ASK-28'),
    ('T-804', 'My Work', 'Drawer - finished tasks', 'Desktop + Mobile', 'Owner / Sales / Production / Finance', 'Reopen for the doer, asker, manager and owner; not for a helper', 'Permissions', 'PASS', 'reopen hidden on a finished task the person only helped on', 'ASK-28'),
    ('T-805', 'My Work', 'See all tasks is for seeing', 'Desktop 1440x900', 'Sales (mocked See all tasks)', 'An unrelated task opens with notes only', 'Permissions', 'PASS', 'no stage, Complete or people controls', 'ASK-28'),
    ('T-806', 'My Work', 'Bulk Complete skips what you may not finish', 'Desktop 1440x900', 'Sales', 'Selecting a task you do and one you help on completes only yours and says 1 skipped', 'Permissions', 'PASS', 'one PATCH, answered locally; toast names the skip', 'ASK-28'),
    ('T-807', 'Tasks API', 'Stuck work - the ladder', 'Backend (unit)', '-', 'Overdue, 1 day, 2 days manager, 4 days owner; what a task is stuck on; who you escalate to', 'Notifications', 'PASS', 'levels 1,2,3,3,4 for 0-4 days; approval / waiting / work; reporting manager else owner', 'ASK-28'),
    ('T-808', 'Tasks API', 'Real saves - reminders reach the right person', 'Backend (isolated DB)', 'Owner / Sales / Ops / Finance / Production', 'Each overdue task notifies exactly the people for its step', 'Notifications', 'PASS', 'doer + helper; waited-on colleague; approver only; manager at 2 days; owner when no manager; owner alert at 4 days', 'ASK-28'),
    ('T-809', 'Tasks API', 'Real saves - Escalate goes to the manager first', 'Backend (isolated DB)', 'Ops (has a manager) / Sales (none)', 'The follow-up goes to the reporting manager, or the owner when there is none', 'Notifications', 'PASS', 'Amit to Priya; Priya to the owner', 'ASK-28'),
    ('T-810', 'Decision Desk', 'Slipping shows escalations and reports work', 'Backend (isolated DB)', 'Sales (a manager)', 'Escalations to me appear; reports tasks appear from 2 days late, not before', 'Desk', 'PASS', 'Escalated by Amit card; Reports to you on the 2-day task; the 1-hour task absent', 'ASK-28'),
    ('T-811', 'My Work', 'Escalate names who it goes to', 'Desktop + Mobile', 'Owner / Sales / Production / Finance', 'The Escalate form says your manager by name, or the owner when you have none', 'Notifications', 'PASS', 'matches the real Reporting Manager field per role; Post refused in the browser', 'ASK-28'),
    ('T-812', 'Decision Desk', 'Slipping follows the ladder on real data', 'Backend (read-only probe)', 'Owner / Sales / Production / Finance', 'Every overdue card is a task I asked for or a report task 2+ days late; the owner keeps all', 'Desk', 'PASS', 'new Slipping code run on real data with a read-only database handle, card by card', 'ASK-28'),
    ('T-813', 'Decision Desk', 'Capture proposes, creates nothing', 'Backend (isolated DB)', 'Owner', 'A capture stores a proposal; no task, workflow, calendar entry or note exists before approval', 'Decisions', 'PASS', 'proposal: 1 task to the named person, 1 workflow with amount, 1 meeting, 1 reminder, 1 note; all collections empty', 'ASK-32'),
    ('T-814', 'Decision Desk', 'Approve creates what was proposed', 'Desktop 1440x900', 'Owner', 'Approve creates the task, workflow, meeting, reminder and note, links the task to its workflow, and says so', 'Decisions', 'PASS', 'toast Approved - 1 task and 1 workflow created; task ready for Priya; approving again refused 409', 'ASK-32'),
    ('T-815', 'Decision Desk', 'Only the decider decides', 'Backend (isolated DB)', 'Finance / named approver', 'Someone who is not the named approver or an owner is refused; the named approver can open and decide', 'Decisions', 'PASS', '403 names who decides; approver opens the decision; decided_by recorded', 'ASK-32'),
    ('T-816', 'Decision Desk', 'Reject creates nothing, deletes no work', 'Desktop 1440x900', 'Owner', 'Rejecting a new decision creates nothing; an older one cancels waiting tasks and keeps work under way', 'Decisions', 'PASS', 'toast Rejected - nothing was created; older decision: in-progress task kept, blocked task cancelled', 'ASK-32'),
    ('T-817', 'Decision Desk', 'Desk card and popup show the proposal', 'Desktop 1440x900', 'Owner', 'Card: On approval: 1 task, 1 workflow, amount, oldest first; popup lists every proposed item and who decides', 'Decisions', 'PASS', 'card line and amount; popup: Approving creates..., Goes to Priya Nair with due date, workflow, meeting, reminder, note, Waiting on', 'ASK-32'),
    ('T-818', 'Decision Desk', 'Nothing to decide, repeats flagged', 'Backend (isolated DB) + Desktop', 'Owner', 'A question makes no decision; the same capture twice within a day is marked as a repeat', 'Decisions', 'PASS', 'voice note outcome nothing_to_decide, no decision or inbox item; Possible repeat on the card and a warning in the popup', 'ASK-32'),
    ('T-819', 'My Work', 'Task of an undecided older decision is locked', 'Desktop + Mobile', 'Sales', 'The task shows Waiting for a decision, cannot be started, and links to the decision', 'Decisions', 'PASS', 'banner; PATCH refused 403 naming the decision; link opens the decision popup', 'ASK-32'),
    ('T-820', 'Dex', 'Phone: one spoken decision, one decision', 'Mobile 390x844', 'Owner', 'The recording is held for review and the reviewed words are sent once; the reply says what Dex read', 'Decisions', 'PASS', '1 held upload, 1 submit, no second capture; reply Ready for a decision ... Nothing is created until it is approved', 'ASK-32'),
    ('T-821', 'Dex', 'Phone: Attach and the Send tap', 'Mobile 390x844', 'Owner', 'Attach opens a picker and the file goes with the decision; Send after a recording works on the first tap', 'Decisions', 'PASS', 'file kept as a chip and sent with file_ids; first Send tap sends (was swallowed)', 'ASK-32'),
    ('T-822', 'Dex', 'Desktop /brain keeps an attached file', 'Desktop 1440x900', 'Owner', 'A file attached on /brain rides with the next note instead of being uploaded to nowhere', 'Decisions', 'PASS', 'Attached: bill.png chip; Note sends file_ids and clears it', 'ASK-32'),
    ('T-823', 'Decision Desk', 'Notification opens the decision', 'Desktop 1440x900', 'Owner', 'Clicking a decision notification opens that decision', 'Decisions', 'PASS', 'lands on /inbox?decision=<id> with the popup open (was the Desk without it)', 'ASK-32'),
    ('T-824', 'Decision Desk', 'WhatsApp approve uses the same path', 'Backend (isolated DB)', 'Finance reviewer', 'Approving a WhatsApp capture creates the work with the reviewer choices and records the decision like the Desk does', 'Decisions', 'PASS', 'decided_by the reviewer, timeline Approved - created..., task with the chosen person and priority', 'ASK-32'),
    ('T-825', 'Decision Desk', 'Decision goes to the right person', 'Backend (isolated DB)', 'Sales / Operations / Owner', 'Capturer who may decide keeps it; otherwise their manager when the manager may; otherwise the owner', 'Decisions', 'PASS', 'sales -> finance manager; operations (manager cannot decide) -> owner; owner -> self', 'ASK-32'),
    ('T-826', 'Decision Desk', 'The decider is told, not the capturer', 'Backend (isolated DB) + Desktop', 'Finance manager', 'A new decision notifies the person who decides; nobody is told about their own capture', 'Notifications', 'PASS', 'Decision waiting for you on Sunita; none on Priya', 'ASK-32'),
    ('T-827', 'Decision Desk', 'Capturer follows what they raised', 'Desktop + Mobile', 'Sales', 'Desk lists it as Waiting on Sunita Rao and does not count it; popup shows who decides, no Approve or Reject; server refuses them', 'Decisions', 'PASS', 'cta follow, counter 0, Who decides card, 403 on approve', 'ASK-32'),
    ('T-828', 'Decision Desk', 'Manager decides', 'Desktop 1440x900', 'Finance manager', 'Desk counts it; popup says Waiting on you with Approve and Change who decides; approve works', 'Decisions', 'PASS', 'review card counted; toast Approved - 1 task created', 'ASK-32'),
    ('T-829', 'Decision Desk', 'Approval tells the people involved', 'Backend (isolated DB) + Desktop', 'Sales / Production', 'The capturer hears it was approved; the person the task went to hears Work assigned to you; rejection tells the capturer', 'Notifications', 'PASS', 'approved your decision (Priya); Work assigned to you (Amit); rejected your decision', 'ASK-32'),
    ('T-830', 'Decision Desk', 'Hand a decision to someone else', 'Desktop + Mobile', 'Owner / Finance', 'Owner or the person it waits on picks from people who may decide; timeline and notification; the previous decider can no longer decide', 'Decisions', 'PASS', 'picker lists only people with Approve decisions; Sent to Rajesh Sharma to decide; Sunita refused 403 and sees Waiting on Rajesh Sharma', 'ASK-32'),
    ('T-831', 'Decision Desk', 'Handing over is guarded', 'Backend (isolated DB)', 'Sales / Owner', 'The capturer cannot hand it on; it cannot go to someone who may not decide; not after it is decided', 'Decisions', 'PASS', '403 capturer; 400 naming Operations User; 409 after approval', 'ASK-32'),
    ('T-832', 'Decision Desk', 'Decision screen shows what was said', 'Desktop 1440x900', 'Owner', 'The words, how they came (voice note with language, typed, WhatsApp), a play button and the files', 'Decisions', 'PASS', 'quoted words, Voice note label, audio player after Play; no confidence or type labels', 'ASK-32'),
    ('T-833', 'Decision Desk', 'Pick who does a task Dex could not match', 'Desktop 1440x900', 'Owner', 'A task with nobody asks Pick who does this; picking saves before approval', 'Decisions', 'PASS', 'assignee saved with how=picked; the picked person gets the task on approval', 'ASK-32'),
    ('T-834', 'Decision Desk', 'Change due date and remove items before approving', 'Desktop + Backend (isolated DB)', 'Owner / Finance', 'Due date and removals save; removed items are not created; only the decider may change; bad dates refused; nothing after deciding', 'Decisions', 'PASS', 'due 2026-09-30 kept on the created task; reminder and meeting not created; 403 non-decider; 400 bad date; 409 after approval', 'ASK-32'),
    ('T-835', 'Decision Desk', 'After approving the popup links to the work', 'Desktop 1440x900', 'Owner', 'The popup stays open with links to each created task and the workflows it created or moved', 'Decisions', 'PASS', '3 task links and workflow links', 'ASK-32'),
    ('T-836', 'Workflows', 'Existing card is moved, not duplicated', 'Desktop + Backend (isolated DB)', 'Owner', 'A message about an order on the board proposes that card and approving moves it to the named stage', 'Workflows', 'PASS', 'Toyota order (already on the board) moves to Dispatched; one Toyota card', 'ASK-32'),
    ('T-837', 'Workflows', 'Tasks join their workflow, plain tasks stay plain', 'Desktop + Backend (isolated DB)', 'Owner', 'A task naming a customer/supplier joins that workflow at its stage; others have no workflow', 'Workflows', 'PASS', 'Call Toyota joins the Toyota card at Dispatched; Kapoor task joins the Kapoor card; Clean the warehouse has none', 'ASK-32'),
    ('T-838', 'Workflows', 'Approval moves a new purchase into its approval stage', 'Desktop + Backend (isolated DB)', 'Owner / Finance manager', 'A new workflow before its approval stage moves into it on approval, for a manager too', 'Workflows', 'PASS', 'dye purchase at approved after owner and manager approvals', 'ASK-32'),
    ('T-839', 'My Work', 'Task links back to its decision', 'Desktop 1440x900', 'Owner', 'The task drawer says From decision with a link', 'Workflows', 'PASS', 'From decision: Dispatch the Toyota order and buy dye', 'ASK-32'),
    ('T-840', 'Workflows', 'Workflow card links back to its decision', 'Desktop 1440x900', 'Owner', 'The card on the board says From decision with a link', 'Workflows', 'PASS', 'link on the dye purchase card', 'ASK-32'),
    ('T-841', 'Decision Desk', 'Phone decision screen edit rows fit', 'Mobile 390x844', 'Owner', 'The person picker and due date fit a phone screen', 'Decisions', 'PASS', 'no page or dialog overflow', 'ASK-32'),
    ('T-842', 'Dex', 'Dex chat proposal creates nothing until approved', 'Backend (isolated DB)', 'Owner', 'A task Dex proposes in chat is a proposal with a named approver; no task exists', 'Decisions', 'PASS', 'proposal with 1 task; 0 tasks in the database', 'ASK-32'),
...
ASKS = [
    ...
    dict(
        ...
        type='Change request',
        prio='High',
        what='PROGRESS 2026-09-14 - PHASE 7 STUCK WORK REACHES THE MANAGER FIRST BUILT (plan 7.1-7.3, D7 yes; founder call: too many escalations reached the owner - the manager hears first, the owner only after several days). One rule in services/tasks.py (followup_level, stuck_on, escalation_manager_id) read by the reminders, the Escalate button and the Desk. Reminder ladder (services/finance_signals.run_followup): overdue - the people who can move it; 1 day - the same again; 2 days - the doer reporting manager (the owner only when nobody manages the doer; before, the Manager escalation went to the owner); 4 days - the owner alert by email / WhatsApp (was 3 days). Every open stage is scanned now (was To do and In progress only): waiting for approval reminds the approver, not the locked doer; Waiting on a colleague reminds them as well as the doer. Escalate goes to your reporting manager, the owner only if you have none (was always the owner); the form names who. Desk Slipping: a manager also sees direct reports tasks from 2 days late (Reports to you); and escalations and hand-offs now appear there at all - the Desk read action / actor_id while notes are saved as kind / author_id, so none ever showed. Acceptance 15/15 + 5 n/a (backend/scripts/ux_task_stuck_0914.py, writes blocked: owner, sales, production, finance on desktop, sales on a phone - the Escalate text names the manager or the owner per the real Reporting Manager field, Post refused in the browser) + Desk probe 4/4 + 4 n/a (backend/scripts/probe_desk_slipping_0914.py: the new Slipping code run read-only on real data for the four demo seats, every card checked against the rule; the local backend was not restarted with Phase 7 because its reminder sweep runs on the production database) + 4 unit tests (backend/tests/test_task_followup.py) + the ladder test updated (test_s7_finance_formula) + 1 real-save journey (test_task_management_e2e.py::test_stuck_work_reaches_the_right_person: every step, approval, Waiting on, no manager, owner alert, Escalate both ways, Desk). Full suite 1452 passed, 40 skipped; failures: the known SSRF test, and test_starter_seat_cap_blocks_eleventh, which passed when re-run alone. Ladder, escalation and e2e tests 47/47 single-process.\nPROGRESS 2026-09-14 - ITEM 7 WHO MAY CHANGE A TASK BUILT (plan 6.7; founder calls: helpers do not mark a task done; only the person who asked or the owner changes Needs proof; See all tasks is for seeing and notes, not editing; the approver notes and approves, it does not edit the work). Before, any signed-in member could change any task through PATCH /tasks/{id}, even one they could not open, and could switch proof off. Now services/tasks.task_edit_rights + edit_refusal and frontend lib/taskAccess.taskEditRights: stage, progress and Waiting on - doer, helpers, the person who asked, the manager of someone on it, the owner; mark done, cancel, reopen - the same minus helpers; doer, team and helpers - the person who asked, the manager, Manage Team, the owner (the doer no longer changes helpers); priority - the person who asked, the manager, the owner; Needs proof - the person who asked and the owner. A value sent unchanged needs no right; someone with no right is refused whatever they send, with the reason, nothing saved. Notes now also open to the named approver and See all tasks holders. Drawer (desktop and phone) shows exactly those controls: a helper sees Kiran marks this done in place of Complete; the person who asked and the manager now get stage and Complete (they were note-only); hand-off stays with whoever is on the task. Bulk Complete and bulk Reassign skip tasks the person may not change and say how many. Acceptance 96/96 (backend/scripts/ux_task_edit_0914.py, writes blocked: owner, sales, production, finance manager on desktop and phone, one made-up task per relationship - doer, helper, asker, approver, waited on, team colleague, unrelated, manager, finished as doer and as helper - plus sales with a mocked See all tasks and a bulk Complete that sends only the task the person does) + 6 unit tests (backend/tests/test_task_access.py) + 1 real-save journey (test_task_management_e2e.py::test_who_may_change_a_task). Re-runs: ux_task_asked_0914 20/20, ux_task_team_0914 21/21, ux_task_stages_0914 13/13, ux_task_approval_stage_0914 19/19, ux_task_people_0914 10/10, ux_task_phone_0914 21/21. Full suite 1452 passed, 40 skipped (item 7 and Phase 7 together); failures: the known SSRF test, and test_starter_seat_cap_blocks_eleventh, which passed when re-run alone (load while browser checks ran). The first browser run caught a leftover reference to a removed variable that broke every task card; fixed before this run. Flutter app still offers Complete to helpers; the server refuses it.\nPROGRESS 2026-09-14 - TK-08 TASK ACCESS BUILT (plan Phase 6; founder calls: finance does NOT see all tasks; who may edit a task through PATCH /tasks/{id} is deferred to a later discussion). (6.1) New permissions tasks_assign_any (Assign tasks to anyone) and tasks_view_all (See all tasks), off for every role by default, owner has them, ticked per person in Team - Access; brain_export stays out of the frontend list on purpose (RBAC-10, the plan note to add it was wrong). (6.2) POST /tasks needs the tasks permission. (6.3, D1) Without Assign tasks to anyone a person may give work to themselves, their own team and their direct reports - as doer, team to route to or helper - on create, on changing people and on reassign; the server refuses with who and why and saves nothing; New Task, Add a person and bulk reassign list only who is allowed (an empty helper list says so). Manage Team alone does not widen it. (6.4) All Tasks for the owner and See all tasks holders: GET /tasks?mine=false returns everything for them and any task opens; desktop switcher, phone view sheet and ?view=all follow it. (6.5) One rule list: services/tasks.py can_see_all_tasks / can_assign_person / can_assign_team and frontend lib/taskAccess.js; /auth/me sends effective_permissions (company role settings included). Acceptance 49/49 (backend/scripts/ux_task_access_0914.py, writes blocked: owner, sales, production, finance manager; desktop and phone; views, Assign to and helper lists exactly per rule, no 403s; a mocked grant of both permissions to sales; the two toggles in the Team member dialog, off by default) + 7 unit tests (backend/tests/test_task_access.py) + 3 real-save journeys (14/14 in test_task_management_e2e.py: every refusal and the grant, create without tasks, see all opt-in with finance refused) + the existing permission suites (167 unit tests green). Re-runs: ux_newtask_form_0914 13/13 + 3 n/a, ux_task_links_0914 23/23, ux_task_phone_0914 21/21, ux_task_team_0914 21/21, ux_person_filter_0913 34/34, ux_task_asked_0914 20/20. Full suite 1441 passed; failures: the known SSRF test, and test_retail_high_volume_outstanding_aggregation and test_every_tenant_collection_isolates_A_from_B, which passed when re-run alone (load while browser checks ran). Not run: an owner with a permission removed (needs a test database).\nPROGRESS 2026-09-14 - TK-07 STAGES AND FLAGS BUILT (plan Phase 3, D6: screens change, stored statuses stay, no migration). People see To do (todo, blocked) / Doing (in_progress, waiting, review) / Done / Cancelled on cards, the drawer, the requester line and toasts; the desktop status picker and the phone segmented bar offer To do and Doing only. WAITING ON replaces the Waiting status: Waiting on someone in the drawer (desktop and phone) takes a colleague or a typed name (e.g. a supplier), stored as waiting_on {user_id or null, name, since, set_by} with status waiting; the card flag says Waiting on Kumar Fabrics with the days; the drawer says since when with Stop waiting; a colleague waited on is notified and may open the task and answer with a note; any other status change ends the wait; the start-approval lock still applies. Approval stays the lock flag from TK-05 and Under review is reached only through approval to close. My Work Status filter: To do, Doing, Waiting on someone, Needs approval, Overdue, Due today, Done (old ?status=blocked or review links open Needs approval). Counts that read status now treat waiting and review as open work: Operating Score, calendar, dashboard, Brief delayed and its To do / Doing counters and drill-downs, task prioritisation, leave impact. FOR YOKESH (Dex, not changed): services/ai/agent_tools.py and routers/brain.py still count open/overdue as todo/blocked/in_progress - add waiting and review. PHASE 7: overdue reminders still scan todo/in_progress only. The Flutter app still shows the stored status words (works unchanged). Acceptance 13/13 (backend/scripts/ux_task_stages_0914.py, writes blocked; filter counts To do 116, Doing 13, Needs approval 39 match the server; the waiting journey on one made-up task with PATCHes answered by the script) + 10 unit tests (backend/tests/test_task_stages.py) + 2 real-save journeys (11/11 in test_task_management_e2e.py: waiting on a colleague and a supplier, notification, note-only access, ended by a status change, Stop waiting, refusals; the start lock). Re-runs: ux_person_filter_0913 34/34, ux_task_asked_0914 20/20, ux_task_links_0914 23/23, ux_task_approval_stage_0914 19/19, ux_task_phone_0914 21/21, ux_task_people_0914 10/10, ux_task_team_0914 21/21.\nPROGRESS 2026-09-14 - TK-06 WHO IS ON A TASK BUILT (plan Phase 2, founder call: keep it as simple as possible - a task has a Doer, Helpers, Asked by and an optional Approver). (2.1) Supporting employee removed: no stored task used it (0 of 185, read-only check), so nothing to move; the server no longer saves support_id or lets it open a task or its activity; older app builds that still send it are not refused (the field is ignored); gone from the web drawer and the Flutter New Task form (mobile/lib/screens/work_screen.dart, not built here). (2.2) The drawer people section says Asked by (You when it was you). (2.3) It names the approver and the moment (Approval before work starts / before it is marked done, or anyone with approval access). (2.4) A team task routed to the member with the fewest open tasks stores auto_assigned; New Task says Assigned to Priya: fewest open tasks in Sales; the drawer says Picked automatically: fewest open tasks in Sales; any later reassign clears it. (2.5) Status changes and overdue reminders now reach everyone on the task (approved, changes requested, clarification and comments already did). Acceptance 10/10 (backend/scripts/ux_task_people_0914.py, writes blocked, one made-up task for the auto-assigned drawer, New Task POST answered by the script) + 3 new real-save journeys in backend/tests/test_task_management_e2e.py (9/9 in total: old apps still create and support_id grants nothing; auto_assigned set then cleared on reassign; a helper hears a status change and the overdue reminder). Re-runs: ux_newtask_form_0914 13/13 + 3 n/a, ux_task_team_0914 21/21.\nPROGRESS 2026-09-14 - REMAINING ITEMS CLOSED: (1) PHONE PASS - My Work had a view switcher on the phone only for the owner (My Tasks | All Tasks); nobody could reach Asked by me, My team or Approvals on a phone. Now one view pill for everyone names the view on screen, carries the approvals waiting count, and opens a sheet of the views that person has (My Tasks, Asked by me, My team for managers, All Tasks for the owner, Approvals for approvers); picks go through ?view= so Back works. New Task was already full-screen on the phone after 0eaa411; the approval choice label Before it is marked done was cut off at 390px and now wraps below lg. Acceptance 21/21 (backend/scripts/ux_task_phone_0914.py: owner, sales, finance manager; writes blocked, New Task POST answered by the script). (2) PLAN 4.3 - a named approver must be able to approve: POST /tasks refuses (400, says who and why) an approver who is not the owner and holds no approvals access through own permissions or the company role settings; someone outside the company is still dropped. (3) REAL SAVES - the journeys the browser could only check with writes blocked now run with real saves in an isolated, dropped test database: backend/tests/test_task_management_e2e.py, 6/6 (Asked by me incl. note-only asker; a non-owner named approver approves before work starts; before done: complete, sign-off request and notification, changes requested, complete again, approve closes, reopen needs sign-off again; approver completing closes directly; 4.3 refusal and role-settings grant; My team see, open, note, refused hand-off). Single-process like the Sprint 12 journeys (skipped under xdist): python -m pytest tests/test_task_management_e2e.py -o addopts= -p no:xdist. (4) OLD AUDIT SCRIPTS - ux_lifecycle_mywork.py and ux_gaps_mywork.py create and delete real tasks, so they now refuse to run unless UX_ALLOW_WRITES=1 (the local database is production), and their selectors follow the current My Work (glass dropdowns via option testids, the progress slider, Status: Completed and ?view=all links instead of the old tabs, the bulk reassign picker); not executed here by design. ux_flow_mywork.py, ux_person_filter_0913.py and ux_task_asked_0914.py use the phone view pill. Re-runs: ux_task_asked_0914 20/20, ux_person_filter_0913 33/33, ux_task_approval_stage_0914 19/19, ux_newtask_form_0914 13/13 + 3 n/a. Full backend suite 1425 passed; failures: the known test_website_intel_ssrf_block_and_fallback and test_thirty_parallel_task_creates_no_lost_writes, which only inserts into its own test database and passed when re-run alone (timing).\nPROGRESS 2026-09-14 - TK-04 Links always land BUILT (desktop + phone link; frontend only): BEFORE (baseline, writes blocked) a task link opened only when the task sat in the list on screen - Sunita opening a task of sai (her report, not in her My Tasks) got no drawer, nor for a finished task, nor when already on My Work (a notification clicked from there); ?focus=task:<id> links (Brief blocks, phone) were ignored; a deleted task said Access restricted; Workflows was lost on refresh; Back left My Work instead of returning to the previous view. NOW: ?task=<id> and ?focus=task:<id> open the drawer whatever the view - from the list card when the task is in the list, else drawn from the single-task request; the link opens again when it changes while My Work is open; closing the drawer takes the link out of the address; a missing task says This task no longer exists and a refused one keeps Access restricted, both with Dismiss; every view writes ?view=mine|asked|team|all|approvals|workflows and pushes a history entry, and the page follows Back and Forward; ?view=all and ?view=mine links work. Acceptance 23/23 (backend/scripts/ux_task_links_0914.py, writes blocked, REAL data: Sunita Rao and sai; owner; a production user refused with 403; one phone check), run after rebasing onto 0eaa411 (glass pass) and afc6136 (UI-SCALE). Re-runs on the merged page: ux_task_asked_0914 20/20, ux_task_approvals_0914 21/21, ux_task_team_0914 21/21, ux_task_approval_stage_0914 19/19, ux_person_filter_0913 33/33, ux_newtask_form_0914 13/13 + 3 n/a. Script upkeep in the same push: the merged page draws its first cards about 3.5 s after load and redraws about 1 s after Clear (traced), so settle() in the links, asked and person-filter scripts now waits for real content and the person filter waits for the card count to change and hold; the asked clears-url check accepts view=mine; the approval-stage and New Task form scripts follow 0eaa411 (no More options row: those three checks report n/a; GlassSelect dropdowns are driven by their option testids). Bisect (MyWork.js from afc6136, without TK-04): the person-filter failures showed there too; the one asked failure that did not repeat without TK-04 was a half-loaded page (traced with TK-04: all 53 cards 3.5 s after reload) and passes once the script waits. Traced separately: Clear filters restores all 129 cards about 1 s after the click.\nPROGRESS 2026-09-14 - TK-03 My team BUILT (desktop, founder call D3: owner keeps All Tasks, managers get My team): a manager is anyone named as a Reporting Manager (Team page field, until now used only for leave). GET /tasks?view=team = tasks whose doer or a helper is one of my direct reports (services/tasks.task_list_query, 9 unit tests, backend/tests/test_task_view_team.py).My Work: My team in the desktop switcher only for a non-owner with reports; kept in the URL (?view=team), not saved as default; Department, Person (lists the team, no Me) and Status filters work; AI priority off; own empty state; a My team link opened by someone without reports lands on My Tasks. The manager can open a report task and its activity (GET /tasks/{id} and /activity now allow the doer or helper manager) and leave a note; if the task is outside the manager department the drawer is note-only (same as Asked by me), inside it the existing department rule still gives full controls. Acceptance 21/21 (backend/scripts/ux_task_team_0914.py, writes blocked, REAL data: Sunita Rao manages sai, 16 tasks; the other-department note-only drawer uses one made-up task) + ux_task_asked_0914 20/20 re-run. Demo data has only one manager, so a manager of several people is covered by unit tests only.\nPROGRESS 2026-09-14 - TK-05 Approval moment BUILT (desktop, plan Phase 4, founder call D2): the creator picks when a task is approved - New Task, More options, Needs approval: No / Before work starts / Before it is marked done (stored as approval_stage start or close; older approval tasks count as start). Before work starts is the lock tasks always had. Before it is marked done leaves the work open: Complete (proof still required) moves the task to Under review with approval pending and notifies the approver (Approval needed to close, opens Approvals); Approve closes it and runs the close hooks (workflow advance, Brain record, invoice draft - moved into one helper so both paths share them); Request changes sends it back to In progress with the reason shown to the doer; moving the status back withdraws the request; a person who may approve closes it directly; a checklist reaching 100 percent follows the same rule. GET /tasks?view=approvals holds a before-done task only while it waits, so the Approvals count and the Desk card include both kinds. Cards say Approval to start / Approval to close / Needs approval to close; the approver box and the doer banners say which moment it is. Acceptance 19/19 (backend/scripts/ux_task_approval_stage_0914.py, writes blocked, test tasks made up by the script, real approvals feed read-only) + 17 unit tests (backend/tests/test_task_approval_stage.py); ux_newtask_form_0914 16/16 and ux_task_approvals_0914 21/21 re-run. NOT proven live: saving a before-done task and approving it for real - waits on a test database. Open: the server still does not check that a named approver holds approval access (plan 4.3).\nPROGRESS 2026-09-14 - TK-02 Waiting for my approval BUILT (desktop), merged into the Approvals view that 3c23e49 added to My Work: GET /tasks?view=approvals (approval required, not approved, not finished; owner sees all, a named approver sees tasks naming them, an Approve Tasks holder also sees tasks naming nobody - the same rule as _can_approve_task) now feeds the Tasks tab of that view AND the Task approvals card on the Desk (both read /tasks?mine=false before, which for a non-owner is their own lane only, so a named approver outside it saw nothing). The Approvals button carries the pending count and also shows for a named approver without approval access; list is oldest first; the approval-requested notification opens /my-work?view=approvals&task=<id> inside Approvals. Acceptance 21/21 (backend/scripts/ux_task_approvals_0914.py, writes blocked, Approve answered by the script) + 8 unit tests (backend/tests/test_task_view_approvals.py). NOT proven live: a non-owner approver approving a real task - no demo user holds approval access or is named on a task, so that path was checked with a mocked list.\nPROGRESS 2026-09-14 - TK-01 Asked by me BUILT (desktop): GET /tasks?view=asked (created by me, not mine to do: I am neither the doer nor a helper); My Work switcher gets Asked by me (owner: My Tasks | Asked by me | All Tasks; staff: My Work | Asked by me), kept in the URL (?view=asked) and never saved as the default; Department, Person (no Me option) and Status filters work on it; AI priority is off there; its own empty state; the person who asked can leave a note on the task (server + form), hand-off and escalate stay with the doer; a phone link lands on the list. Acceptance 20/20 (backend/scripts/ux_task_asked_0914.py, writes blocked) + 13 unit tests (backend/tests/test_task_view_asked.py). NOT proven live: a non-owner creating a task and then seeing it - no non-owner in the demo company has created one, and creating one writes to the production database; the note-only form was checked with a mocked list. TK-02..TK-04 still to do.\nPlan: docs/TASK_MANAGEMENT_PLAN.md Phase 1; checklist: https://claude.ai/code/artifact/53a2b23e-66a5-4eeb-bfb3-52a87ce08043. TK-01 Asked by me (tasks I created for others). TK-02 Waiting for my approval (named approver, or anyone with approvals when none is named). TK-03 My team (doer or helper reports to me, via reporting_manager_id). TK-04 /my-work?task= opens the task even when it is not in the current view.',
    ...
    dict(
        id='ASK-32',
        section='Decision Desk',
        item='Decision flow Phase 1: nothing is created until a decision is approved',
        type='Change request',
        prio='High',
        what='PROGRESS 2026-09-15 - PHASES 3 AND 4 BUILT (Yokesh: keep the decision screen simple - no AI understanding or confidence, plain task rows, pick the person when unclear, change person / due date / remove, show what was said; workflows must not duplicate, tasks join the workflow they belong to, ad hoc tasks stay plain, workflows move by themselves). PHASE 3: (3.1) What was said - the words, Voice note / Typed / WhatsApp with language, Play the voice note (GET /voice-notes/{id}/audio), files sent with it. (3.2) Plain rows - who, due, priority, Part of <workflow>; type chip removed. (3.3) Before approving, whoever decides picks who does a task (unmatched names ask Pick who does this; the task assignment rule applies), changes the due date, removes a task, workflow, meeting, reminder or note (PATCH /decisions/{id}/proposal/tasks/{key}, DELETE /decisions/{id}/proposal/{kind}/{key}); each change on the timeline. (3.4) After approving the popup stays with links to the created tasks and workflows. PHASE 4: (4.1) Proposals check open cards on the board: same type and same customer/supplier (Toyota = Toyota Kirloskar Pvt Ltd) or contact -> the existing card, moving to a later stage when one is named, never a duplicate. (4.2) A task naming a customer/supplier joins that workflow (proposed or on the board) at its current stage; other tasks stay plain (the join-the-first-workflow fallback is gone). No recurring-task feature exists to cover. (4.3) On approval: new workflows fire their first stage automation; named existing cards step to the named stage; a workflow right before its approval stage moves into it for every pipeline and for a manager approval too; results on the timeline. (4.4) Task drawer and workflow card link back to the decision. (4.5) Dex chat propose_task stores a proposal with a named approver. Checks: browser 26/26 with REAL saves in the throwaway database (backend/scripts/ux_decision_screen_0915.py, desktop and phone; the voice-note audio bytes mocked) + 5 real-save journeys added to backend/tests/test_decision_flow_e2e.py (existing card moved not duplicated, tasks joined or left plain, manager approval moves a new purchase into its approval stage, proposal edits, what was said, Dex chat proposal). Full suite 1517 passed, 55 skipped; the only failure is the known SSRF test. The single-process decision journeys (backend/tests/test_decision_flow_e2e.py, 15) run separately and pass. API parity baseline regenerated for the three new routes.\nPROGRESS 2026-09-15 - PHASE 2 BUILT (who decides and who is told). (2.1) Every new decision names who decides (services/decision_flow.route_approver, stored with approver_route): the person who captured it when they may approve decisions, else their reporting manager when the manager may, else the owner; a company with several owners keeps it with every owner. The 51 older waiting decisions with no approver were NOT backfilled (production data) and show Waiting on an owner. (2.2) One permission routes and approves: decisions_approve (owner, or effective permissions incl. company role settings); routing used to check approvals. (2.3) Told: the decider when a decision arrives (not the capturer); on approval each person a task went to (Work assigned to you) and the capturer (approved your decision); on rejection the capturer; a WhatsApp capture approved on the spot tells nobody it waits. (2.4) The capturer follows it: their Desk lists it as Waiting on Sunita, not counted in their Decisions number; the popup shows who decides with no Approve or Reject; the Dex reply says Sent to Sunita to decide. (2.5) Change who decides from the popup: an owner or the person it waits on picks from people who may decide (GET /decisions/{id}/approvers, POST /decisions/{id}/approver); timeline Sent to X to decide; the new person is told. Checks: browser 29/29 with REAL saves in the throwaway database (backend/scripts/ux_decision_routing_0915.py: Priya reports to Sunita, desktop and phone) + 2 real-save journeys added to backend/tests/test_decision_flow_e2e.py (9 in the file, production-client detector: none). Full suite 1517 passed, 49 skipped; the only failure is the known SSRF test (API parity baseline regenerated for the two new routes).\nPROGRESS 2026-09-15 - PHASE 1 BUILT (docs/DECISION_DESK_PLAN.md; Yokesh: the Decide flow is for the owner and, in some cases, managers; tasks are created only when the decision is approved). '
             '(1.1) A capture now stores a PROPOSAL on the pending decision - tasks with who they go to and when, workflows with pipeline, stage, customer or supplier and amount, meetings, reminders, company notes - and creates nothing; Approve creates all of it (services/voice.materialize_proposal) and links the tasks to their workflow. '
             '(1.2) A task an older decision created blocked cannot be started before that decision is approved (server 403 + My Work banner with a link to the decision). '
             '(1.3) Approve and reject only while undecided and only by the named approver or an owner (409 / 403); the decision records who decided; reject creates nothing and, for older decisions, cancels waiting tasks and removes only untouched auto-created work, never work under way. '
             '(1.4) A capture with nothing to act on makes no decision (voice note outcome nothing_to_decide) and Dex says so; a failed capture says why (AI switched off). '
             '(1.5) The same capture again within a day is flagged: Possible repeat on the Desk card, a warning in the popup. '
             '(1.6) Phone: a spoken decision is held, its words come back for review and are sent as ONE note (it used to make two decisions); Attach works and a photo or file is read with the decision; the Send tap after a recording no longer needs pressing twice; /brain keeps an attached file for the next note. '
             '(1.7) A decision notification opens the decision. (1.8) Approving a WhatsApp capture goes through the same approve path. (1.9) /dex/capture checks the voice capture permission. '
             'Desk card: On approval: N tasks, N workflows; oldest first (it sorted the text Waiting N days); amount from the proposed workflows. Popup: Approving creates ..., each proposed task, workflow, meeting, reminder and note, Waiting on the approver, Approved by the decider. '
             'Checks: browser 38/38 with REAL saves in a throwaway database (backend-scratch, email and WhatsApp mocked; speech, AI and file storage mocked in the phone and /brain checks) - backend/scripts/ux_decision_flow_0915.py; 7 real-save journeys (backend/tests/test_decision_flow_e2e.py); older decision tests updated; a production-client detector confirmed no test touched the live database. Full suite 1514 passed, 47 skipped; failures: the known SSRF test, and the 3 API-parity tests because a route was added (POST /voice-notes/{id}/submit) - baseline regenerated, parity 4/4 after.',
        why='Dex made live workflows, calendar entries, reminders and company notes before anyone approved; rejecting deleted work already under way; 23 of 54 waiting decisions were questions or greetings; one spoken decision became two; Attach did nothing; notifications did not open the decision.',
        code='services/voice.py (build_proposal, materialize_proposal, process_voice_note hold), services/decision_flow.py, routers/decisions.py, routers/voice_notes.py (hold, submit), routers/dex.py, services/captures.py, routers/tasks.py (decision lock), routers/desk.py, services/enrich.py, frontend DecisionDialog.js, useDexCapture.js, useDexConversation.js, DexChat.jsx, DexFab.jsx, DexStage.jsx, notif.js, MyWork.js',
        dep='ASK-23, ASK-27',
        status='In progress',
    ),
]
