...
async def _compute(plan, retrieved, scope):
    ...
    start, end = _date_range(plan.get("date_preset"))
    kpis, columns, rows, cites = [], [], [], []

    def uname(uid):
        return (umap.get(uid) or {}).get("name") or "Unassigned"

    # ---- TASK completion / on-time analysis (join with activity task_done) ----
    ...
    # ---- Employees ranked by overdue / open tasks ----
    if entity == "employees":
    ...
    # ---- Tasks (generic list / overdue / by status) ----
    if entity == "tasks":
        now = datetime.now(timezone.utc).isoformat()
        st = plan.get("status")
        filtered = []
    ...
    # ---- Decisions / Workflows / Leaves / Memory (generic list) ----
    if entity == "decisions":
        for d in recs[:200]:
    ...
    return kpis, {"columns": columns, "rows": rows, "total_rows": 0}, cites
