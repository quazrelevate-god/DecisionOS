...
const inp = "w-full border border-hairline px-2.5 py-1.5 text-sm text-label focus:outline-none focus:shadow-xs bg-surface";
  ...
  return (
    <>
      <button type="button" data-testid={testid} onClick={view} title="View attachment"
        className="flex items-center gap-1.5 text-xs font-semibold border border-hairline px-3 py-1.5 hover:bg-primary hover:text-white transition-colors rounded-md">
      ...
      {isImage && (
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent className="border border-hairline rounded-md max-w-3xl p-2" data-testid={`${testid}-lightbox`}>
            <DialogHeader><DialogTitle className="tracking-tight text-sm truncate">{filename || "Attachment"}</DialogTitle></DialogHeader>
...
function SelectField({ label, value, onChange, options, optLabels }) {
  ...
  return (
    <label className="block">
      <span className="text-label text-text-secondary text-[10px]">{label}</span>
          ...
          {isOwner ? (
            <>
              <div className="flex items-center justify-between mb-2">
                <p className="text-label font-bold text-status-completed-fg flex items-center gap-1.5">
              ...
              {(logs || []).length === 0 ? (
                <div className="border border-hairline p-3 text-xs text-text-secondary rounded-md" data-testid="whatsapp-logs-empty">
                  No inbound messages logged yet. Scan the QR and send a test message — it will appear here within seconds. If nothing shows after sending, the webhook isn't reaching the app (check Meta → WhatsApp → Configuration: callback URL, <span className="text-label">messages</span> field subscribed, and app published).
                </div>
              ) : (
                <div className="space-y-1.5 max-h-72 overflow-y-auto" data-testid="whatsapp-logs-list">
                  {logs.map((l, i) => (
                    <div key={`${l.created_at || ""}-${l.from || ""}-${i}`} data-testid={`whatsapp-log-${i}`} className="border border-hairline p-2 flex items-start gap-2 flex-wrap rounded-md">
                      <Chip value={l.status} className={WA_STATUS_STYLE[l.status] || "bg-surface-sunken text-text"} />
                      <span className="text-label text-text-secondary">{l.mtype || "—"}</span>
                      <span className="text-xs text-label">{l.from || "unknown"}</span>
                      <span className="text-xs text-text-secondary flex-1 min-w-[120px]">{l.summary || l.reason || ""}</span>
                      <span className="text-label text-text-secondary">{timeAgo(l.updated_at || l.created_at)}</span>
                    </div>
                  ))}
                </div>
              )}
    ...
    <div className="rounded-lg border border-hairline bg-surface p-5 mb-8" data-testid="ingest-review-panel">
      <div className="flex items-start justify-between gap-3 mb-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="font-black tracking-tight text-lg">Review extracted data</span>
          </div>
          ...
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            ...
            {ingestion.confidence != null && (
              <span className="text-label text-text-secondary">confidence {Math.round(ingestion.confidence * 100)}%</span>
            )}
          </div>
        </div>
        <div className="flex gap-2">
          ...
          <button data-testid="ingest-cancel-button" onClick={onCancel} className="px-4 py-2 text-sm font-semibold border border-hairline hover:bg-surface-hover transition-colors rounded-md">
            Discard
          </button>
          <button data-testid="ingest-file-button" disabled={filing || total === 0} onClick={fileIt}
            className="flex items-center gap-2 bg-primary text-primary-foreground px-5 py-2 text-sm font-semibold border border-hairline hover:shadow-sm transition-all disabled:opacity-50 rounded-md">
      ...
      {total > 0 && (
        <div className="border-l-4 border-hairline-strong bg-background p-3 mb-5 flex items-start gap-2" data-testid="ingest-direction-banner">
          ...
          <div>
            <p className="text-sm font-bold tracking-tight">{hint.label}</p>
      ...
      {ownHits.length > 0 && (
        <div className="border border-hairline bg-status-pending-bg p-3 mb-5 flex items-start gap-2 rounded-md" data-testid="ingest-owncompany-warning">
      ...
      {(records.contacts || []).length > 0 && (
        <div className="mb-5" data-testid="review-contacts">
          <p className="text-label text-primary-text mb-2 flex items-center gap-1"><UsersThree size={14} weight="bold" /> {L.customer_plural} & {L.vendor_plural} ({records.contacts.length})</p>
          <div className="space-y-2">
            {records.contacts.map((c, i) => (
              <div key={c._key} className={`border p-3 grid grid-cols-2 md:grid-cols-4 gap-2 relative ${isOwnCompany(c.name, ownNorm) ? "border-status-pending-line bg-status-pending-bg/20" : "border-hairline"} rounded-md`} data-testid={`review-contact-${i}`}>
                ...
                <button onClick={() => removeItem("contacts", i)} data-testid={`remove-contact-${i}`} className="absolute -top-2 -right-2 w-6 h-6 flex items-center justify-center border border-hairline bg-surface hover:bg-destructive-tint transition-colors rounded-md"><Trash size={12} weight="bold" /></button>
              </div>
            ))}
          </div>
        </div>
      ...
      {(records.invoices || []).length > 0 && (
        <div className="mb-5" data-testid="review-invoices">
          <p className="text-label text-primary-text mb-2 flex items-center gap-1"><Receipt size={14} weight="bold" /> Invoices & Bills ({records.invoices.length})</p>
          <div className="space-y-2">
            {records.invoices.map((inv, i) => {
              ...
              return (
              <div key={inv._key} className={`border p-3 grid grid-cols-2 md:grid-cols-6 gap-2 relative ${needsBucket ? "border-hairline-strong border-2 bg-primary-tint" : "border-hairline"} rounded-md`} data-testid={`review-invoice-${i}`}>
                ...
                {isPurchase && (
                  <label className="block" data-testid={`review-invoice-bucket-${i}`}>
                    <span className="text-label text-text-secondary text-[10px]">Book as {needsBucket && <span className="text-primary-text">• pick one</span>}</span>
                ...
                <button onClick={() => removeItem("invoices", i)} data-testid={`remove-invoice-${i}`} className="absolute -top-2 -right-2 w-6 h-6 flex items-center justify-center border border-hairline bg-surface hover:bg-destructive-tint transition-colors rounded-md"><Trash size={12} weight="bold" /></button>
              </div>
            );})}
          </div>
        </div>
      ...
      {(records.payments || []).length > 0 && (
        <div className="mb-5" data-testid="review-payments">
          <p className="text-label text-primary-text mb-2 flex items-center gap-1"><CurrencyCircleDollar size={14} weight="bold" /> Payments ({records.payments.length})</p>
          <div className="space-y-2">
            {records.payments.map((p, i) => (
              <div key={p._key} className="border border-hairline p-3 grid grid-cols-2 md:grid-cols-5 gap-2 relative rounded-md" data-testid={`review-payment-${i}`}>
                ...
                <button onClick={() => removeItem("payments", i)} data-testid={`remove-payment-${i}`} className="absolute -top-2 -right-2 w-6 h-6 flex items-center justify-center border border-hairline bg-surface hover:bg-destructive-tint transition-colors rounded-md"><Trash size={12} weight="bold" /></button>
              </div>
            ))}
          </div>
        </div>
      ...
      {(records.tasks || []).length > 0 && (
        <div className="mb-1" data-testid="review-tasks">
          <p className="text-label text-primary-text mb-2 flex items-center gap-1"><ListChecks size={14} weight="bold" /> Follow-up tasks ({records.tasks.length})</p>
          <div className="space-y-2">
            {records.tasks.map((t, i) => (
              <div key={t._key} className="border border-hairline p-3 flex items-center gap-2 relative rounded-md" data-testid={`review-task-${i}`}>
                <input className={inp} value={t.title ?? ""} onChange={(e) => setItem("tasks", i, "title", e.target.value)} />
                <button onClick={() => removeItem("tasks", i)} data-testid={`remove-task-${i}`} className="shrink-0 w-8 h-8 flex items-center justify-center border border-hairline bg-surface hover:bg-destructive-tint transition-colors rounded-md"><Trash size={12} weight="bold" /></button>
              </div>
            ))}
          </div>
        </div>
  ...
  const tabsEl = (
    <div className="flex gap-2 mb-6" data-testid="capture-main-tabs">
      <button data-testid="capture-maintab-import" onClick={() => setMainTab("import")}
        className={`px-4 py-2 text-sm font-semibold  border-hairline transition-all ${mainTab === "import" ? "bg-primary text-primary-foreground shadow-xs" : "bg-surface hover:bg-surface-hover"} rounded-md`}>
        Import
      </button>
      <button data-testid="capture-maintab-review" onClick={() => setMainTab("review")}
        className={`px-4 py-2 text-sm font-semibold  border-hairline transition-all flex items-center gap-2 ${mainTab === "review" ? "bg-primary text-primary-foreground shadow-xs" : "bg-surface hover:bg-surface-hover"} rounded-md`}>
        Review Queue
        {pendingCount > 0 && (
  ...
  return (
    <div>
      <PageHeader eyebrow="Data Input" title="Ingest">
        <a href="#records" className="hidden md:flex items-center gap-2 px-4 py-2 text-sm font-semibold border border-hairline hover:bg-surface-hover transition-colors rounded-md">
      ...
      {canIngest ? (
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          <label data-testid="upload-document-zone" className={`rounded-lg border border-hairline bg-surface p-6 flex flex-col items-center justify-center text-center cursor-pointer shadow-hover ${uploading ? "opacity-60 pointer-events-none" : ""}`}>
            ...
            <span className="font-bold tracking-tight">PDF / Photo</span>
          ...
          <label data-testid="upload-csv-zone" className={`rounded-lg border border-hairline bg-surface p-6 flex flex-col items-center justify-center text-center cursor-pointer shadow-hover ${uploading ? "opacity-60 pointer-events-none" : ""}`}>
            ...
            <span className="font-bold tracking-tight">CSV / Excel</span>
      ...
      {uploading && (
        <div className="rounded-lg border border-hairline bg-surface p-8 mb-8 flex items-center justify-center gap-3" data-testid="ingest-loading">
          ...
          <span className="tracking-tight text-sm">Reading document with AI…</span>
        </div>
      )}
      ...
      {(history || []).length > 0 && (
        <div className="mb-8">
          <p className="text-label text-primary-text mb-3">Recent uploads</p>
          <div className="space-y-2">
            {history.map((h) => (
              <div key={h.id} data-testid={`ingestion-row-${h.id}`} className="border border-hairline bg-surface p-3 flex items-center gap-3 flex-wrap rounded-md">
                ...
                {h.status === "filed" && h.created_counts && (
                  <span className="text-label text-text-secondary">{h.created_counts.contacts}C · {h.created_counts.invoices}I · {h.created_counts.payments}P · {h.created_counts.tasks}T</span>
                )}
                {h.status === "review" && !active && (
                  <button data-testid={`resume-ingestion-${h.id}`} onClick={() => setActive(h)} className="text-xs font-semibold border border-hairline px-2 py-1 hover:bg-primary hover:text-white transition-colors rounded-md">Review</button>
                )}
                {h.status === "failed" && (
                  <p data-testid={`ingestion-error-${h.id}`} className="basis-full text-xs text-primary-text text-label break-words">
      ...
      <div id="records">
        <div className="flex gap-2 mb-4">
          <button data-testid="records-tab-invoices" onClick={() => setTab("invoices")} className={`px-4 py-2 text-sm font-semibold  border border-hairline transition-colors ${tab === "invoices" ? "bg-primary text-primary-foreground" : "bg-surface hover:bg-surface-hover"} rounded-md`}>Invoices & Bills</button>
          <button data-testid="records-tab-payments" onClick={() => setTab("payments")} className={`px-4 py-2 text-sm font-semibold  border border-hairline transition-colors ${tab === "payments" ? "bg-primary text-primary-foreground" : "bg-surface hover:bg-surface-hover"} rounded-md`}>Payments</button>
        </div>

        {tab === "invoices" && (
          (invoices || []).length === 0 ? <EmptyState title="No invoices yet" hint="Upload an invoice or a sales CSV to get started." /> : (
            <div className="overflow-x-auto border border-hairline bg-surface rounded-md" data-testid="invoices-table">
              <table className="w-full text-sm">
                <thead className="bg-primary text-primary-foreground"><tr>
                  {["Type", "Number", "Party", "Amount", "Due", "Status"].map((h) => <th key={h} className="text-left px-3 py-2 text-label">{h}</th>)}
                </tr></thead>
                <tbody>
                  {invoices.map((inv) => (
        ...
        {tab === "payments" && (
          (payments || []).length === 0 ? <EmptyState title="No payments yet" hint="Upload a payment receipt or a payments CSV." /> : (
            <div className="overflow-x-auto border border-hairline bg-surface rounded-md" data-testid="payments-table">
              <table className="w-full text-sm">
                <thead className="bg-primary text-primary-foreground"><tr>
                  {["Direction", "Amount", "Party", "Method", "Reference", "Date"].map((h) => <th key={h} className="text-left px-3 py-2 text-label">{h}</th>)}
                </tr></thead>
                <tbody>
                  {payments.map((p) => (
