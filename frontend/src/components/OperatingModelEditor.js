                    ...
                    <div className="space-y-1.5">
                      {(s.tasks || []).map((t, ti) => (
                        <div key={t._uid} className="flex items-center gap-1.5" data-testid={`op-stage-task-${pi}-${si}-${ti}`}>
                          ...
                          <button onClick={() => delStageTask(pi, si, ti)} title="Delete task" className="p-1 text-muted-foreground hover:text-brand-red"><Trash size={12} weight="bold" /></button>
                        </div>
                      ))}
                    </div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {s.side_effects.map((se, ei) => (
                          <span key={se._uid} data-testid={`op-stage-side-effect-${pi}-${si}-${ei}`}
                            ...
                            {se.kind}
                            <button onClick={() => delSideEffect(pi, si, ei)} title="Remove" className="text-muted-foreground hover:text-brand-red">
