// KR-4 · the Karma kit, one door. Pages import from "components/karma" so a
// primitive can move files without touching forty call sites.
export { IconChip } from "./IconChip";
export { ArrowButton } from "./ArrowButton";
export { BigNumeral } from "./BigNumeral";
export { CountUp } from "./CountUp";
export { PillNav } from "./PillNav";
export { DarkBand } from "./DarkBand";
export { DotProgress, MiniBars, CircleDots, TinySpark } from "./MiniViz";
// KR-8 additions
export { ArcGauge } from "./ArcGauge";
export { KDeltaChip } from "./KDeltaChip";
export { StatTile } from "./StatTile";
export { WideStatCard } from "./WideStatCard";
export { HistoryBand } from "./HistoryBand";
export { DecisionBento } from "./DecisionBento";
export { InsightWell } from "./InsightWell";
// KR-9 additions
export { ScoreMeter } from "./ScoreMeter";
