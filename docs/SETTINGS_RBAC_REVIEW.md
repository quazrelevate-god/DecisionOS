...
- [x] **P1:** *(Fixed 2026-09-16: after adding someone with a password, a welcome message to copy or send on WhatsApp — where to sign in and with which email, never the password. "Get invite link" only shows for an invite not yet accepted — superseded the same day by Sakthivel's Team redesign (8b60707), where the invite icon sits beside the name for every member but an owner, and opens their form to add a mobile number when they have none. Reviewed again 2026-09-16 and his rule kept: the link cannot sign anyone in on its own — the code goes to the member's own mobile — and it covers two cases the narrower rule left stranded, a member with no number on file and one who never worked out how to sign in. What the narrower rule was really guarding against is fixed at the source instead: `/auth/invite/{token}/start` now keeps the ordinary 30s OTP cooldown, so a link forwarded around WhatsApp can no longer text someone on a loop, and a re-tap inside that window still returns what the device needs rather than an error.)* People who sign in with a password get no invite or hand-off. "Get invite link" also shows for people who are already active.
...
## Found while re-checking the Team page (2026-09-16)

- [x] **P0: an invited member could not use the app at all.** *Fixed the same day.* A member is created as a `pending` membership row and nothing ever moved that row to `active`; since 59e9684 a pending row also closes the old-account fallback in `get_current_user`. So the invitee tapped their link, entered the texted code, landed on the Decision Desk — and every call after that came back 403 "You no longer have access to this workspace", which threw them back to the sign-in screen. Signing in now IS accepting the invite (`accept_pending_membership`), on the code door and the password door both. Suspended and removed rows are untouched: those are decisions about the person, not a state they can log their way out of. The seat is recounted rather than reserved at the door, so a workspace that filled up after the invite went out still lets in someone who was already invited. Walked in the browser end to end: `backend/scripts/ux_invite_accept_0916.py` (17/17), which fails on exactly these four checks when the fix is taken out.
- [x] **P0: a team manager could take over a teammate's account.** *Fixed 2026-09-16.* `PATCH /users/{id}` let anyone with Manage team change another member's mobile number, and the number is the login — so a manager could point a colleague's account at their own phone and sign in as them, which was the "Manage team cannot raise access" rule (1fca68d) walked around. An owner was already protected. The rule now: a team manager may ADD a number where there is none — someone without one cannot sign in at all, and the redesign's invite icon depends on it — and only an owner may change one that is already set, the same line email edits draw. One mobile, one member inside a workspace (the same number in another workspace is still fine, for the consultant who serves two). A manager still edits everything else about a member.
- [x] **P1: a person could not keep their own details current.** *Fixed 2026-09-16.* Settings › Your Profile held name and mobile only: job title was a manager's to set, email said "can't be changed here", and there was nowhere to say what you look after. It now holds name, job title, what you handle, mobile and email — and nothing on that card touches role, permissions or the reporting line, which stay a manager's call on the Team page. The email is the sign-in, so a change is confirmed by the current password (or, for a member who signs in by mobile and has no password, a code texted to them and checked the way the sign-in door checks it), the address must be free, and it goes back to unverified until they click the link. What someone handles shows on their Team profile in their own words.
