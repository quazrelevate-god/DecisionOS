...
def sweep_dialog(p, vp, opener, idx, errs, blocked, shot_fn):
    ...
    for b in ordered:
        ...
        if not ident:
            continue
        # Switching language re-renders the whole app in another language for
        # every later assertion, so record the options and leave them alone.
        if ident.startswith("lang-option-"):
            rec(vp, f"  [{opener} > {ident}] offered", None,
                "not pressed -- it would re-render the app for every later check")
            continue
        if b["disabled"]:
