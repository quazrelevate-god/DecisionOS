...
export function OsReady({ industryLabel, summary, onContinue }) {
  ...
  return (
    <div className="space-y-5" data-testid="onboarding-os-ready">
      <div className="border border-hairline bg-primary text-primary-foreground p-5 rounded-md">
        <CheckCircle size={28} weight="fill" className="text-primary-text mb-2" />
        <h3 className="text-2xl font-black tracking-tighter leading-tight" data-testid="os-ready-title">
      ...
      <div className="grid grid-cols-2 gap-3" data-testid="os-summary">
        {cards.map((s) => (
          <div key={s.label} data-testid={`os-summary-${s.label.split(" ")[0].toLowerCase()}`} className="border border-hairline bg-surface p-4 rounded-md">
            <s.icon size={20} weight="bold" className="text-primary-text mb-1.5" />
            <p className="text-3xl font-black leading-none">{s.n}</p>
            <p className="text-label text-text-secondary mt-1">{s.label}</p>
          </div>
        ))}
      </div>
      <button data-testid="os-ready-continue" onClick={onContinue} className="w-full flex items-center justify-center gap-2 bg-primary text-primary-foreground font-semibold py-3 border border-hairline hover:shadow-sm transition-all rounded-md">
