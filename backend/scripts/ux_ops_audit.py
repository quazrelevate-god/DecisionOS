"""Ops section -- exhaustive UI/UX audit, desktop then mobile.
...
ROUTE = "/operating-score"
OUT = REPO / ".audit-artifacts" / "ux" / "ops"
...
def audit(browser, vp, dims, mobile):
    ...
    sampled, seen_pref = [], {}
    for c in ctrls:
        m = _re.match(r"^(operating-emp-|operating-cat-|ops-action-)", c.get("tid") or "")
        if m:
...
def main():
    ...
    print(f"\n  report: {(OUT / 'ops-audit.json').relative_to(REPO)}")
    return 0
