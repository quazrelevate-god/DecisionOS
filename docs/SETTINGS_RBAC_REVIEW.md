...
- **Open: a team manager can take over a teammate's account.** `PATCH /users/{id}` lets anyone with Manage team change another member's mobile number, and the number is the login — so a manager can point a colleague's account at their own phone and sign in as them, which is the "Manage team cannot raise access" rule (1fca68d) walked around. An owner is already protected. The fix that keeps the flows we have: a team manager may ADD a number where there is none (the redesign's invite icon depends on that), and only an owner may change one that is already set — the same line email edits draw today.
