"""Object-storage provider adapter (Epic 8 Sprint 6 -- from services/obj_store.py).

Emergent Object Storage wrapper: persistent file storage (survives redeploys),
addressed by a UUID path with the DB as the source of truth. Uses EMERGENT_LLM_KEY,
auto-refreshes an expired storage key once per call, and treats delete as
best-effort. Imports stdlib + core + httpx only.
"""
import os
import asyncio
import httpx
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as c:
            ...
            if r.status_code in (200, 204, 404):
                return True
            logger.warning(f"storage.delete_object({path}) returned HTTP {r.status_code}")
            return False
    except Exception as e:
        logger.warning(f"storage.delete_object({path}) failed: {e}")
        return False
