  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <button data-testid="new-workflow-button" className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 text-sm font-semibold uppercase tracking-wider border border-hairline hover:shadow-sm transition-all">
          <Plus size={16} weight="bold" /> {t("workflows.new")}
        </button>
      </DialogTrigger>
      <DialogContent className="border border-hairline rounded-md">
        <DialogHeader><DialogTitle className="uppercase tracking-tight">{t("workflows.dlg_title", { type: typeLabel })}</DialogTitle>
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="text-label uppercase text-text-secondary">{contactLabel}</label>
        ...
        <DialogFooter>
          <button data-testid="wf-create-submit" onClick={create} className="bg-primary text-primary-foreground px-5 py-2 text-sm font-semibold uppercase tracking-wider border border-hairline hover:shadow-xs transition-all">{t("workflows.create")}</button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
export default function Workflows({ embedded = false }) {
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap border border-hairline mb-6 w-fit">
        {pipelines.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)} data-testid={`workflow-tab-${t.key}`}
            ...
            <span className={`block text-[10px] uppercase tracking-widest ${activeKey === t.key ? "text-white/60" : "text-text-secondary"}`}>{t.sub}</span>
          </button>
        ))}
      </div>
            ...
            return (
              <div key={stg.key} className="w-64 shrink-0 border-r border-hairline last:border-r-0" data-testid={`stage-column-${stg.key}`}>
                <div className="px-3 py-2 border-b border-hairline bg-background sticky top-0">
                  <p className="text-label uppercase">{stg.label}</p>
                  ...
                  {cards.map((w) => {
                    ...
                    return (
                      <div key={w.id} id={`workflow-card-${w.id}`} data-testid={`workflow-card-${w.id}`} className={`border border-hairline p-3 shadow-hover bg-surface transition-all ${w.id === focusWf ? "ring-4 ring-ring ring-offset-2" : ""}`}>
                        ...
                        {w.amount != null && <p className="text-label uppercase text-xs mt-1">{money(w.amount, tenant?.currency)}</p>}
                        {updAt && (
                          <p className="text-label uppercase text-text-secondary mt-2 flex items-center gap-1" data-testid={`workflow-updated-${w.id}`} title={fullTime(updAt)}>
                        ...
                        {!isLast && (
                          <button onClick={() => advance(w)} data-testid={`advance-workflow-${w.id}`}
                            className="mt-3 w-full flex items-center justify-center gap-1 border border-hairline py-1.5 text-xs font-semibold uppercase tracking-wider hover:bg-surface-hover transition-colors">
