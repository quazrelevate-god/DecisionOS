    ...
    <div className="py-1" data-testid="language-options">
      {LANGUAGES.map((l) => (
        <button key={l.code} onClick={() => choose(l.code)} data-testid={`lang-option-${l.code}`}
          ...
          {current === l.code && <Check size={16} weight="bold" className="text-primary-text" />}
        </button>
      ))}
    </div>
  );
  ...
  if (variant === "inline") {
    return (
      <div className="flex flex-wrap gap-2" data-testid="language-inline">
        {LANGUAGES.map((l) => (
          <button key={l.code} onClick={() => choose(l.code)} data-testid={`lang-option-${l.code}`}
            className={`flex items-center gap-2 px-4 py-2 text-sm rounded-lg border transition-all ${current === l.code ? "bg-primary text-primary-foreground border-hairline" : "bg-surface border-hairline hover:bg-black/[0.04]"}`}>
