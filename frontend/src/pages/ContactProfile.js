...
const Section = ({ icon: Icon, title, count, children }) => (
  <div className="mb-8">
    <div className="flex items-center gap-2 mb-3">
      ...
      {count != null && <span className="text-label uppercase text-text-secondary">({count})</span>}
    </div>
    {children}
  </div>
);
  ...
  return (
    <div className="overflow-x-auto border border-hairline bg-surface">
      <table className="w-full text-sm">
        <thead className="bg-primary text-primary-foreground"><tr>{head.map((h) => <th key={h} className="text-left px-3 py-2 text-label uppercase">{h}</th>)}</tr></thead>
        <tbody>{rows}</tbody>
      </table>
    </div>
  );
...
export default function ContactProfile() {
  ...
  if (isLoading) return <div className="text-label uppercase text-sm uppercase tracking-widest py-20 text-center">Loading profile…</div>;
  ...
  return (
    <div>
      <button onClick={() => navigate("/contacts")} data-testid="profile-back" className="flex items-center gap-2 text-sm font-semibold uppercase tracking-wider mb-4 hover:text-destructive-text transition-colors">
      ...
      <div className="rounded-lg border border-hairline bg-surface p-6 mb-6" data-testid="profile-header">
        ...
        <h1 className="text-4xl font-black uppercase tracking-tighter">{c.name}</h1>
        ...
        <div className="flex flex-wrap gap-4 mt-3 text-sm">
          ...
          {c.tax_id && <span className="text-label uppercase text-text-secondary">{c.tax_id}</span>}
        </div>
      </div>
      ...
      <div className="rounded-lg border border-hairline bg-surface p-6 mb-8" data-testid="relationship-card">
        <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-extrabold uppercase tracking-tight">Relationship Intelligence</h2>
          </div>
          <button
          ...
          <div>
            ...
            {(rel.signals || []).length > 0 && (
              <div className="flex flex-wrap gap-2 mt-3" data-testid="rel-signals">
                {rel.signals.map((s, i) => (
                  <span key={`sig-${i}-${s.slice(0, 20)}`} className="inline-block px-2 py-0.5 text-xs border border-hairline bg-surface">{s}</span>
                ))}
              </div>
            )}
          </div>
        ...
        {invoices.length === 0 ? <p className="text-sm text-text-secondary">No bills recorded.</p> : (
          <Table head={["Number", "Amount", "Date", "Due", "Status"]} rows={invoices.map((inv) => (
            <tr key={inv.id} data-testid={`profile-invoice-${inv.id}`} className="border-t border-hairline">
              ...
              <td className="px-3 py-2 text-label uppercase text-xs">{inv.date || "—"}</td>
        ...
        <Section icon={Truck} title="Pending Deliveries" count={pending_deliveries.length}>
          {pending_deliveries.length === 0 ? <p className="text-sm text-text-secondary">Nothing pending.</p> : (
            <div className="space-y-2">{pending_deliveries.map((w) => (
              <div key={w.id} className="border border-hairline bg-surface p-3 flex items-center justify-between gap-2">
        ...
        <Section icon={TrendUp} title="Price History" count={price_history.length}>
          <Table head={["Item", "Rate", "Date"]} rows={price_history.map((h, i) => (
            <tr key={`${h.item || "row"}-${h.date || ""}-${i}`} className="border-t border-hairline">
              ...
              <td className="px-3 py-2 text-label uppercase text-xs">{h.date || "—"}</td>
            </tr>
          ))} />
        </Section>
      )}
      ...
      <Section icon={Warning} title="Complaints" count={complaints.length}>
        {complaints.length === 0 ? <p className="text-sm text-text-secondary">No complaints logged.</p> : (
          <div className="space-y-2">{complaints.map((cp) => (
            <div key={cp.id} data-testid={`profile-complaint-${cp.id}`} className="border border-hairline bg-surface p-3">
